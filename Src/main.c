...
extern uint8_t uart_receive_frame[19];
extern uint8_t svf10_resister_frame[16];
...
//  #endif
  ...
  SVF_DLY2X_OSC = OSC_10MHZ;     // 1
  SVF_S_GAIN = GAIN_2X;        // 01
  SVF_ADC_RSEL = RSEL_INTERAL;  // 0
  SVF_DYL2X = DYL2X_15NS;       // 1
  SVF_CLK_SEL = CK_SYS;         // 00
  SVF_PERIOD = PERIOD_000;      // 100
  SVF_ISOL = ISOL_25LINE;       // 00
  SVF_SL_TIME = SL_1000MS;      // 3
  SVF_NAVI = NAVI_OFF;
  SVF_SCK_UT = SCK_UT_MODE_01;
...
//    #endif
 ...
 //   SVF10_Sreg_Read_Creg_Set(svf10_tatus_buffer);
 //   HAL_Delay(1);
...
//      image_quality(svf10_origine_buffer, svf10_image_data);
      hist_ep(svf10_origine_buffer, svf10_image_data, svf10_average-(svf10_std_dev/2), svf10_average+(svf10_std_dev/2), 70, 185, EP_OPTION_LINEAR);
      noise_filter(1, svf10_origine_buffer, svf10_image_data, svf10_result_data);
